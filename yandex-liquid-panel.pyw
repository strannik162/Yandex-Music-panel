...
class CustomSlider(QWidget):
    ...
    def paintEvent(self, event):
        ...
        bg_rect = QRectF(0, y, rect.width(), h)
        ...
        if self.max_val > 0:
            ...
            progress = min(max(self.value / self.max_val, 0), 1)

            fg_w = rect.width() * progress

            if fg_w > 0:

                fg_rect = QRectF(0, y, fg_w, h)

                painter.setBrush(QBrush(QColor(255, 255, 255, 180)))

                painter.drawRoundedRect(fg_rect, h/2, h/2)
    ...
    def _animate(self):
        ...
        if self.playing:
            ...
            for i in range(self.bars):
                ...
                self.target_amplitudes[i] = random.uniform(0.1, 1.0)

        else:

            for i in range(self.bars):

                self.target_amplitudes[i] = 0.05
    ...
    def paintEvent(self, event):
        ...
        for i in range(self.bars):
            ...
            painter.drawRoundedRect(
                ...
                QRectF(x, h - bh, bar_w - 2 * padding, bh),
...
class LiquidMusicPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.info_row.addWidget(self.app_label)

        self.info_row.addWidget(self.visualizer, 1)



        text_block.addLayout(self.info_row)
    ...
    def update_media(self, data: dict):
        ...
        self.visualizer.update_state(playing)

        self.slider.set_value(data.get("pos", 0), data.get("duration", 0))
