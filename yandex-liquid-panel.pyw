        ...
        try:
            ...
            if action == "play_pause":
                ...
                try:
                    ...
                    await session.try_change_playback_position_async(timedelta(microseconds=ticks/10))
                except Exception:
                    pass
    ...
    def _update_anim(self):
        if not self.playing:
            for i in range(self.bars):
                self.values[i] = max(0.1, self.values[i] * 0.85)
        else:
            for i in range(self.bars):
                target = random.uniform(0.1, 0.8)
                if i < 6:
...
class CustomSlider(QWidget):
    ...
    def set_timeline(self, pos, duration, playing):
    ...
    def _interpolate(self):
        if self.playing and not self.dragging:
            self.pos_val = min(self.duration, self.pos_val + 0.1)
            self.update()
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton and self.dragging:
            self.dragging = False
            self.action_requested.emit("seek", self.pos_val)

    def _update_from_mouse(self, point):
        rel = max(0, min(1, point.x() / self.width()))
        self.pos_val = rel * self.duration
        self.update()
    ...
    def paintEvent(self, event):
        ...
        h = self.height()
        cy = h / 2
        progress = self.pos_val / self.duration
        ...
        painter.drawRoundedRect(0, cy - 2, w * progress, 4, 2, 2)

        # Handle
        hx = w * progress
        painter.setBrush(QColor(255, 255, 255, 245))
        painter.drawEllipse(QPointF(hx, cy), 6, 6)
...
class LiquidMusicPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.main_row.addLayout(content_layout)
    ...
    def toggle_mode(self):
        ...
        geo = self.geometry()
        new_geo = QRect(geo.x(), geo.y(), geo.width(), target_h)
        ...
        self.geo_anim.setStartValue(geo)
    ...
    def update_media(self, data: dict):
        ...
        self.slider.set_timeline(data.get("position", 0), data.get("duration", 1), playing)
    ...
    def hide_panel(self):
        ...
        if self.manual_pos:
            self._animate_to(self.x(), self.saved_manual_y, 0.0)
            try:
                self.pos_anim.finished.disconnect()
            except Exception:
                pass
            self.pos_anim.finished.connect(lambda: self.move(self.x(), -5000) if not self.visible_panel and self.manual_pos else None)
        else:
            screen = QApplication.screenAt(QCursor.pos()) or QApplication.primaryScreen()
            geo = screen.availableGeometry()
            target_x = geo.x() + geo.width() // 2 - self.width() // 2
