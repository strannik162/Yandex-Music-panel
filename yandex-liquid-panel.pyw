                ...
                if timeline:
                    ...
                    if pos is not None:
                        if hasattr(pos, "total_seconds"):
                            pos_sec = pos.total_seconds()
                        elif hasattr(pos, "duration"):
                            pos_sec = pos.duration / 10000000.0
                        else:
                            pos_sec = float(pos)

                    dur = getattr(timeline, "end_position", None)
                    if dur is not None:
                        if hasattr(dur, "total_seconds"):
                            dur_sec = dur.total_seconds()
                        elif hasattr(dur, "duration"):
                            dur_sec = dur.duration / 10000000.0
                        else:
                            dur_sec = float(dur)
            except Exception:
                pass
...
class SpectrumVisualizer(QWidget):
    def __init__(self, parent=None):
        ...
        self.bars_count = 32
        self.heights = [0.0] * self.bars_count
        self.playing = False
    ...
    def _tick(self):
        ...
        if self.playing:
            for i in range(self.bars_count):
                if i < 10:
                    thump = math.sin(self.phase * 1.5) * 0.4 + 0.5
                    noise = random.uniform(0.0, 0.3)
                    target = thump + noise
                elif i > 22:
                    flicker = random.uniform(0.1, 0.9)
                    target = flicker
                else:
                    mid = math.sin(self.phase * 0.8 + i * 0.3) * 0.3 + 0.4
            ...
            for i in range(self.bars_count):
                self.heights[i] = max(0.0, self.heights[i] * 0.8)
    ...
    def paintEvent(self, event):
        ...
        h = self.height()
        gap = 1.5
        total_gaps = gap * (self.bars_count - 1)
        bar_w = (w - total_gaps) / self.bars_count
        ...
        for i in range(self.bars_count):
            ...
            alpha = int(120 + 135 * self.heights[i])
            if self.playing:
...
class LiquidCard(QFrame):
    ...
    def mouseMoveEvent(self, event):
        if self.drag_start_pos is not None and (event.buttons() & Qt.LeftButton):
            ...
            self.window().move_panel_manually(new_pos)
            event.accept()
        else:
            super().mouseMoveEvent(event)
...
class LiquidMusicPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        bottom_row.addSpacing(78)

        self.bottom_vbox = QVBoxLayout()
        self.bottom_vbox.setContentsMargins(0, 0, 0, 0)
        self.bottom_vbox.setSpacing(4)
        self.bottom_vbox.addWidget(self.visualizer)
        self.bottom_vbox.addWidget(self.slider)

        bottom_row.addLayout(self.bottom_vbox)
    ...
    def update_media(self, data: dict):
        ...
        self.slider.set_range(data.get("position", 0.0), data.get("duration", 0.0))

    def _elide(self, text, label, width):
        metrics = label.fontMetrics()
        return metrics.elidedText(text, Qt.ElideRight, width)

    def move_panel_manually(self, pos):
    ...
    def toggle_mode(self):
        ...
        self.setMinimumHeight(min(PANEL_H_SMALL, PANEL_H_LARGE))
        self.setMaximumHeight(max(PANEL_H_SMALL, PANEL_H_LARGE))
        ...
        pos = self.pos()
        start_rect = QRect(pos.x(), pos.y(), PANEL_W, self.height())
        target_rect = QRect(pos.x(), pos.y(), PANEL_W, target_h)
        ...
        self.geo_anim.finished.connect(on_geo_finished)
        self.geo_anim.start()
